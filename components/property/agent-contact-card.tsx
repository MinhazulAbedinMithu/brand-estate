...
export function AgentContactCard({ lister, propertyTitle, className }: AgentContactCardProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-border-default/50 bg-bg-surface p-5 sm:p-6 shadow-sm relative overflow-hidden",
        className
      )}
    >
      {/* Wrapper to blur content if NID is not verified */}
      <div className={cn("space-y-5", !isNidVerified && "blur-md select-none pointer-events-none")}>
      ...
      {/* NID Lock Overlay */}
      {!isNidVerified && (
        <div className="absolute inset-0 z-20 rounded-2xl bg-bg-surface/50 backdrop-blur-xs flex flex-col items-center justify-center p-5 text-center">
          <div className="h-10 w-10 rounded-xl bg-accent-primary/10 border border-accent-primary/20 flex items-center justify-center text-accent-primary mb-3 shadow-sm animate-pulse">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          <span className="text-[12px] font-bold text-text-primary tracking-wide">Inquiry Locked</span>
          <p className="text-[10px] text-text-secondary mt-1.5 leading-relaxed max-w-[190px] mx-auto mb-4">
            {currentUser 
              ? "Please submit NID verification in your settings to contact agents."
              : "Please register and verify NID to contact agents."}
          </p>
          {currentUser ? (
            <Link href="/dashboard/profile" className="w-full">
              <Button className="w-full h-9 rounded-full bg-accent-primary hover:bg-accent-primary-hov text-white text-[11px] font-bold">
                Verify NID
              </Button>
            </Link>
          ) : (
            <div className="grid grid-cols-2 gap-2 w-full">
              <Link href="/login" className="w-full">
                <Button variant="outline" className="w-full h-9 rounded-full text-text-secondary text-[11px] font-bold border-border-default">
                  Sign In
                </Button>
              </Link>
