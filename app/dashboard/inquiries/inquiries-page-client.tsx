    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/60 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">Communicate with real estate agents regarding properties of interest</p>
        </div>
        <div className="flex gap-1.5 bg-[#0F1829] border border-slate-800/80 p-1 rounded-xl">
        ...
        <div className="rounded-2xl border border-slate-800/60 bg-[#0A101C] overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-800/80 bg-[#0F1829]/50 text-slate-400 font-extrabold uppercase tracking-wider select-none">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300 font-medium">
                {filteredInquiries.map((inq) => (
                  <tr key={inq.id} className="hover:bg-[#0F1829]/25 transition-colors">
                    ...
                    <td className="py-4 px-5 max-w-xs">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="font-bold text-white truncate block max-w-[180px] sm:max-w-xs">
                          {inq.propertyTitle}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-2">
                        <Avatar className="h-6.5 w-6.5 border border-slate-700">
                          <AvatarImage src={inq.agentAvatar} alt={inq.agentName} />
                          <AvatarFallback className="bg-slate-800 text-[10px] text-white">
                            {inq.agentName[0]}
                          </AvatarFallback>
                        </Avatar>
                        <span className="font-bold text-slate-200">{inq.agentName}</span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-5 text-right">
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="h-8 rounded-lg text-[11px] text-accent-primary hover:text-white hover:bg-slate-800 font-bold px-3.5"
                      >
                        Inspect Details
                      </Button>
                    </td>
      ...
      <Sheet open={!!selectedInquiry} onOpenChange={(open) => !open && setSelectedInquiry(null)}>
        <SheetContent className="w-full sm:max-w-md bg-[#0A101C] border-l border-slate-800 text-slate-200 flex flex-col justify-between p-6">
          <SheetHeader className="pb-4 border-b border-slate-800/60">
            <SheetTitle className="text-white text-left font-heading text-lg font-bold">Inquiry Status Details</SheetTitle>
          ...
          {selectedInquiry && (
            <div className="flex-1 overflow-y-auto py-6 space-y-6 custom-scrollbar pr-1">
              ...
              <div className="p-3.5 rounded-2xl bg-[#0F1829] border border-slate-800 flex gap-3">
                <img
                  src={selectedInquiry.propertyImage}
                  alt={selectedInquiry.propertyTitle}
                  className="h-14 w-14 rounded-xl object-cover shrink-0 border border-slate-850"
                />
                <div className="min-w-0 flex-1 space-y-1">
                  <span className="text-[10px] font-bold text-accent-primary uppercase tracking-wider">Property Inquiry</span>
                  <h4 className="text-xs font-bold text-white leading-snug truncate">{selectedInquiry.propertyTitle}</h4>
                  <Link
                ...
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] text-slate-500 font-semibold">{selectedInquiry.submittedDate}</span>
                    </div>
                    <div className="rounded-2xl rounded-tl-none bg-accent-primary/8 border border-accent-primary/15 p-3.5 text-xs text-slate-200 leading-relaxed">
                  ...
                  <div className="flex gap-3 animate-fade-in">
                    ...
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[10px] text-slate-500 font-semibold">{selectedInquiry.replyDate}</span>
                      </div>
                      <div className="rounded-2xl rounded-tl-none bg-[#0F1829] border border-slate-800 p-3.5 text-xs text-slate-200 leading-relaxed">
                        {selectedInquiry.replyMessage}
                      </div>
                    </div>
                  </div>
          ...
          <div className="border-t border-slate-800/60 pt-4 flex gap-3 mt-4">
            <Button
              ...
              variant="outline"
              className="flex-1 h-10 rounded-xl border-slate-800 hover:bg-slate-800 text-slate-200"
            >
              Close Panel
            </Button>
