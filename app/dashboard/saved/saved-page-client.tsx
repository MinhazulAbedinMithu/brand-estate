    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/60 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">Monitor pricing updates and view features for your favorited homes</p>
        </div>
        <div className="text-xs font-bold bg-[#0F1829] border border-slate-800 px-3.5 py-1.5 rounded-full text-slate-400 select-none">
          Total Favorites: <span className="text-white">{properties.length}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="pl-9 h-10 rounded-xl border-slate-800 bg-[#0A101C] text-sm text-slate-200 placeholder:text-slate-500 focus:ring-accent-primary focus:bg-bg-base transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <select
            ...
            className="h-10 text-xs font-bold text-slate-300 bg-[#0A101C] border border-slate-800 rounded-xl px-3 focus:outline-none focus:ring-1 focus:ring-accent-primary"
          >
      ...
      {paginatedProperties.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-800/80 bg-[#0A101C]/40 p-12 text-center max-w-xl mx-auto flex flex-col items-center justify-center gap-4 mt-8">
          <div className="h-14 w-14 rounded-full bg-slate-900 flex items-center justify-center text-slate-500">
            <Heart className="h-6 w-6" />
          </div>
          <div className="space-y-1">
            <h3 className="font-heading text-base font-bold text-white">No Saved Properties</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedProperties.map((property) => {
              ...
              return (
                <div
                  key={property.id}
                  className="group flex flex-col bg-[#0A101C] border border-slate-800/60 rounded-2xl overflow-hidden hover:border-slate-700 transition-all duration-300 hover:shadow-xl hover:shadow-accent-primary/2"
                >
                  <div className="relative aspect-video w-full overflow-hidden bg-slate-900 shrink-0">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 h-8.5 w-8.5 rounded-full bg-slate-950/80 backdrop-blur-sm border border-slate-800 text-rose-400 hover:bg-rose-500 hover:text-white hover:border-rose-500 flex items-center justify-center shadow transition-all duration-200"
                  ...
                  <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between gap-4">
                    <div className="space-y-1.5">
                      ...
                      <h3 className="text-sm sm:text-base font-bold text-white truncate group-hover:text-accent-primary transition-colors leading-snug">
                        {property.title}
                      </h3>
                    ...
                    <div className="flex items-center justify-between bg-[#0F1829] border border-slate-800/60 p-2.5 rounded-xl text-xs text-slate-400 select-none">
                      {property.propertyCategory !== "commercial" ? (
                        <>
                          <span className="flex items-center gap-1.5 font-medium">
                            <Bed className="h-3.5 w-3.5 text-accent-primary/80" />
                            <span className="font-bold text-white">{property.bedrooms}</span> Bed
                          </span>
                          <span className="flex items-center gap-1.5 font-medium">
                            <Bath className="h-3.5 w-3.5 text-accent-primary/80" />
                            <span className="font-bold text-white">{property.bathrooms}</span> Bath
                          </span>
                          <span className="flex items-center gap-1.5 font-medium">
                            <Ruler className="h-3.5 w-3.5 text-accent-primary/80 rotate-90" />
                            <span className="font-bold text-white">{property.squareFeet.toLocaleString()}</span> ft²
                          </span>
                        </>
                      ) : (
                        <>
                          ...
                          <span className="text-slate-500 font-bold capitalize truncate">
                            Commercial
                          </span>
                        </>
                      )}
                    ...
                    <div className="border-t border-slate-800/60 pt-3 flex items-center justify-between mt-1">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              <Button
                ...
                className="h-9 px-3 rounded-lg border-slate-800 text-slate-300 hover:bg-slate-800"
              >
                Previous
              </Button>
              <div className="text-xs font-semibold text-slate-400 px-3 select-none">
