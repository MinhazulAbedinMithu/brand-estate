...
import { ArrowLeft, ArrowRight, Check, MapPin, CheckCircle2, Plus, Trash, Globe } from "lucide-react";
...
const WIZARD_STEPS = [
  ...
  { id: 6, name: "Review & Save", desc: "Inspect summary and save updates" },
];
...
export function EditListingClient({ property }: EditListingClientProps) {
  ...
  const [syncTitle, setSyncTitle] = React.useState(
    !property.seo || !property.seo.seoTitle || property.seo.seoTitle === property.title
  );
  const [syncDesc, setSyncDesc] = React.useState(
    !property.seo || !property.seo.metaDescription || property.seo.metaDescription === property.description
  );
  const [syncKeywords, setSyncKeywords] = React.useState(
    !property.seo || !property.seo.keywords || property.seo.keywords.join(", ") === (property.amenities ? property.amenities.join(", ") : "")
  );
  ...
  const [form, setForm] = React.useState({
    ...
    ogImageUrl: property.seo?.ogImageUrl || "",
    useCoverAsOg: !property.seo || !property.seo.ogImageUrl || property.seo.ogImageUrl === (property.images ? property.images[0] : ""),
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      ...
      if (form.images.length === 0) {
        toast.error("Validation error", { description: "Please add at least one image in Step 4 before proceeding." });
        return;
      }
    }
  ...
  const handleSaveChanges = async (status: "draft" | "active") => {
    ...
    const payload = {
      ...
      seo: {
        ...
        ogImageUrl: form.useCoverAsOg ? form.images[0] : form.ogImageUrl,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-16">
        ...
        <Button render={<Link href="/owner/listings" />} size="icon-sm" variant="ghost" className="h-9 w-9 text-text-muted hover:text-text-primary hover:bg-bg-elevated rounded-lg">
                      ...
                      onClick={() => {
                        setForm(p => {
                          ...
                          return {
                            ...p,
                            images: nextImages,
                            ogImageUrl: p.useCoverAsOg ? (nextImages[0] || "") : p.ogImageUrl
                          };
                        });
        ...
        {step === 6 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="border border-border-default bg-bg-base rounded-xl p-4.5 space-y-1.5 font-sans select-none shadow-sm text-left">
            ...
            <div className="rounded-xl border border-border-default bg-bg-alt/30 p-5 space-y-4 text-xs font-medium">
              ...
              {form.seoKeywords && (
                <div className="pt-2 border-t border-border-default text-left">
                  <span className="text-[10px] text-text-muted font-bold block uppercase tracking-wider mb-1">Target SEO Keywords</span>
                  <div className="flex flex-wrap gap-1">
                    {form.seoKeywords.split(",").map((k: string) => k.trim()).filter(Boolean).map((k: string) => (
