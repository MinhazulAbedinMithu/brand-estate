    ...
    return (
      <div className="max-w-xl mx-auto py-12 px-6 rounded-2xl border border-border-default bg-bg-surface text-center space-y-6 my-8 shadow-sm">
        ...
        <div className="pt-2 flex justify-center">
          {currentUser.status === "unsubmitted" ? (
            <Button render={<Link href="/agent/submit-docs" />} className="bg-accent-primary text-white hover:bg-accent-primary-hov rounded-full px-6 h-10 font-bold cursor-pointer">
    ...
    <div className="space-y-6 relative pb-20">
      ...
      {filteredListings.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border-default bg-bg-alt/20 p-12 text-center max-w-xl mx-auto flex flex-col items-center justify-center gap-4 mt-6 animate-fade-in shadow-xs">
          ...
          <Button render={<Link href="/agent/listings/new" />} size="sm" className="h-9 rounded-full bg-accent-primary text-white font-bold px-5 mt-2 cursor-pointer">
            Create Listing Now
          </Button>
        </div>
      ) : (
                    ...
                    <tr key={prop.id} className="hover:bg-bg-alt/10 transition-colors">
                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <Button render={<Link href={`/agent/listings/${prop.id}/edit`} />} size="icon-sm" variant="ghost" className="h-8 w-8 text-text-muted hover:text-text-primary hover:bg-bg-elevated rounded-lg cursor-pointer">
