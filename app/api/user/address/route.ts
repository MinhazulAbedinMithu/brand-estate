...
export async function POST(request: NextRequest) {
  try {
    ...
    const { addressLine, addressCity, addressCountry } = body as {
      addressLine?: string;
      addressCity?: string;
      addressCountry?: string;
    };

    if (!addressLine || !addressCity || !addressCountry) {
      return NextResponse.json(
        { status: 'error', error: 'ValidationError', message: 'Address, city, and country are required.' },
        { status: 400 }
      );
    }
    ...
    user.addressLine = addressLine.trim();
    user.addressCity = addressCity.trim();
    user.addressCountry = addressCountry.trim();
