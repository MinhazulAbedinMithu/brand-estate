...
export async function PATCH(
  ...
  try {
    ...
    const { nidStatus, reason } = body as { nidStatus?: string; reason?: string };

    if (!nidStatus || !['unsubmitted', 'pending', 'verified', 'rejected'].includes(nidStatus)) {
      return NextResponse.json(
        { status: 'error', error: 'ValidationError', message: 'Invalid nidStatus value provided.' },
        { status: 400 }
      );
    }
    ...
    user.nidStatus = nidStatus as 'unsubmitted' | 'pending' | 'verified' | 'rejected';
    ...
    return NextResponse.json(
      { status: 'success', message: 'User NID verification status updated successfully.' },
      { status: 200 }
    );
  } catch (err) {
    console.error('[PATCH /api/admin/users/[id]/nid-status]', err);
    return NextResponse.json(
      { status: 'error', error: 'InternalServerError', message: 'Failed to update user NID status.' },
      { status: 500 }
    );
  }
}
