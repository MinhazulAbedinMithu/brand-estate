    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-800/60 pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium font-body">Global operations control, database health metrics and security audit trail</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
        {metrics.map((item, idx) => (
          <div key={idx} className="p-4 rounded-xl border border-slate-800/60 bg-[#0A101C] hover:border-amber-500/25 transition-all">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">{item.label}</span>
            <span className="text-xl font-extrabold font-heading text-white mt-1.5 block">{item.value}</span>
            <span className="text-[10px] text-slate-400 font-semibold block mt-0.5">{item.sub}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-800/60 bg-[#0A101C] p-5 sm:p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800/50 pb-4">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase">Live trail</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[11px] border-collapse">
              ...
              <tbody className="divide-y divide-slate-850 text-slate-350 font-medium font-body">
                {logs.map((log) => (
                  <tr key={log.id} className="hover:bg-amber-500/2 transition-colors">
                    <td className="py-3 px-3">
                      ...
                      <span className="text-[9px] text-slate-500 uppercase block mt-0.5">{log.actorRole}</span>
                    </td>
                    <td className="py-3 px-3 font-semibold text-amber-400">
                      {log.action}
                    </td>
                    <td className="py-3 px-3 truncate max-w-[150px] sm:max-w-xs text-slate-300">
                      {log.target}
                    </td>
        ...
        <div className="rounded-2xl border border-slate-800/60 bg-[#0A101C] p-5 sm:p-6 shadow-sm flex flex-col justify-between">
          <div className="space-y-4">
            <div className="border-b border-slate-800/50 pb-4">
              ...
              <p className="text-xs text-slate-500 mt-0.5">Global system commands triggers</p>
            </div>

            <div className="space-y-3 pt-2">
              <div className="p-3 rounded-xl bg-slate-900/40 border border-slate-800 flex items-start gap-2.5 text-xs text-slate-400">
              ...
              <div className="space-y-3 pt-4">
                <Button
                  ...
                  disabled={flushing}
                  className="w-full h-11 rounded-xl bg-[#0F1829] hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white font-bold gap-2"
                >
          ...
          <div className="text-[10px] text-center text-slate-600 font-bold select-none border-t border-slate-800/60 pt-4 mt-6">
